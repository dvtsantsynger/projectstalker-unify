...
namespace DotNetty.Common.Internal
{
    ...
    public static class PlatformDependent
    {
        ...
        public static void SetMemory(byte[] src, int srcIndex, int length, byte value)
        {
            if (length > 0)
            {
                MemSet(src, srcIndex, length, value);
                //Unsafe.InitBlockUnaligned(ref src[srcIndex], value, unchecked((uint)length));
            }
        }

        public static void MemSet(byte[] array, int srcIndex, int size, byte value)
        {
            if (array == null)
            {
            ...
            int block = 32, index = srcIndex;
            int length = Math.Min(block, size);
